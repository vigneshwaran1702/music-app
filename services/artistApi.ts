...
export const artistApi = {
  ...
  async getArtistById(artistId: string): Promise<Artist | null> {
    ...
    if (artist) {
      try {
        const liveTracks = await jioSaavnApi.searchSongs(artist.name, 20);
        const songs = liveTracks.length > 0 ? liveTracks : CURATED_FEATURED_SONGS.filter((s) => s.artistId === artistId);
    ...
    try {
      ...
      const saavnSongs = await jioSaavnApi.searchSongs(cleanName, 20);
      ...
      return {
        ...
        genres: mbData?.genres?.length ? mbData.genres : ['Kollywood', 'Pop', 'Vocal'],
        bio: mbData?.bio || `${cleanName} - Trending composer and recording artist.`,
